...
class CartoonX:
    ...
    def step(self, std_yl, mean_yl, std_yh, mean_yh, yl, yh, s_yl, s_yh, score, target, num_mask_entries,segmentation_mask):
        """
        args:
...
#         assert len(new_preds.shape) == 2
        ...
        if self.distortion_measure == "label" or self.distortion_measure == "maximize-target":
            ...
            repeat_target=target.repeat(new_preds.shape[0],1,1,1)
            new_scores = torch.gather(self.softmax(new_preds),1,repeat_target)
#             print(score.shape,new_scores.shape,segmentation_mask.shape)
            # Approximate expected distortion with simple Monte-Carlo estimate
            distortion = torch.mean((segmentation_mask*(score - new_scores))**2)
    ...
    def __call__(self, x, target,segmentation_mask):
        """
        args:
        ...
        if self.distortion_measure == "label":
            ...
            score = torch.gather(self.softmax(self.model(x.detach()).detach()),1,target)
        ...
        for i in range(self.num_steps):
            ...
            distortion, sparsity = self.step(std_yl, mean_yl, std_yh, mean_yh, yl,
                                             yh, s_yl, s_yh, score, target, num_mask_entries,segmentation_mask)
