...
class CartoonX:
    def __init__(self, model, device, batch_size, num_steps, step_size, l1lambda, wave, mode, J,
                 ...
                 return_mask=True, return_logs=True):
        """
        args:
    ...
    def step(self, std_yl, mean_yl, std_yh, mean_yh, yl, yh, s_yl, s_yh, score, target, num_mask_entries):
        ...
        # Compute distortion between score for obfuscation z and score for original image
        ...
        return distortion, sparsity, dists
    
    def __call__(self, x, target, scaling='normal', retain_graph=False ):
        """
        args:
        ...
        logs = {"l1-norm": [], "distortion": [], "loss": [], 'score': []}
        ...
        if self.obfuscation_strategy == "gaussian-adaptive-noise":
            ...
            std_yh = []
            mean_yh = []
            if scaling == 'inverse_linear':
                scaling_values = []
                i = 1
                for value in yh:
                    scaling_values.append(i/len(yh))
                    i += 1
                j = 0
                for y in yh:
                    std_yh.append(torch.std(y)*scaling_values[j])
            ...
            if scaling == 'exponential':
                ...
                i = 1
                for value in yh:
                    scaling_values.append(2**(-i/len(yh)))
                    i += 1
                j = 0
                for y in yh:
                    std_yh.append(torch.std(y)*scaling_values[j])
        ...
        for i in tqdm(range(self.num_steps)):
            ...
            """
            #save bottom 80% of mask coefficients and set the rest to zero

            distortion, sparsity, dists = self.step(std_yl, mean_yl, std_yh, mean_yh, yl,
            ...
            # Log loss terms
            logs['score'].append(score.detach())
        ...
        mask = [s_yl.detach().cpu().tolist(), [s.detach().cpu().tolist() for s in s_yh]]
        
        if self.return_logs:
            if self.return_mask:
                return cartoonX, mask, logs, dists
            else:
