...
class CartoonX:
    def __init__(self, model,processor, device, batch_size, num_steps, step_size, l1lambda, wave, mode, J,
    ...
    def step(self, text, std_yl, mean_yl, std_yh, mean_yh, yl, yh, s_yl, s_yh, score, target, num_mask_entries):
        """
        args:
        ...
        z = z.clamp(0,1)*255 # We clamp overflowing values after inverse DWT back to [0,1] 
        print('In step, size of processed image =', z.shape)
        print('text=',text)
        
        z = self.processor(z[0], text, return_tensors="pt")
        # Get model score for the obfuscation z
        new_preds = self.model(**z).logits
        ...
        if self.distortion_measure == "label" or self.distortion_measure == "maximize-target":
            ...
            print('distortion=',distortion, end='\r')
    ...
    def __call__(self, x, text, target):
        """
        args:
        ...
        # print('In __call__, size of processed image =', x.shape)
        # print('text=',text)
        x = self.processor(x[0], text, return_tensors="pt")
        ...
        if self.distortion_measure == "label":
            ...
            score = self.softmax(self.model(**x).logits.detach())[:, target].detach()
            # score = self.model(**x).logits.detach()[:, target].detach()
            ...
            score = self.softmax(self.model(**x.detach()).logits.detach())
        elif self.distortion_measure ==  "kl-divergence":
            score = self.softmax(self.model(**x.detach()).logits.detach())
            assert target is None
        elif self.distortion_measure ==  "weighted-l2":
        ...
        for i in range(self.num_steps):
            ...
            """
            distortion, sparsity = self.step(text, std_yl, mean_yl, std_yh, mean_yh, yl,
